...
class LmergeIntegrationTest(TestCase):
    ...
    def run_integration_test_for_field_inference(self):
        test_directory = os.path.dirname(os.path.abspath(__file__))
        self.test_data_dir = os.path.join(test_directory, 'test_data', 'lmerge')
        input_file = os.path.join(self.test_data_dir, 'field_inference.present.vcf')
        expected_result = os.path.join(self.test_data_dir, 'field_inference.expected.vcf')
        temp_descriptor, temp_output_path = tempfile.mkstemp(suffix='.vcf')
        with os.fdopen(temp_descriptor, 'w') as output_handle:
            # FIXME this is pretty hacky
            temp_handle, sys.stdout = sys.stdout, output_handle
    ...
    def run_integration_test_for_field_inference_no_fields(self):
        test_directory = os.path.dirname(os.path.abspath(__file__))
        self.test_data_dir = os.path.join(test_directory, 'test_data', 'lmerge')
        input_file = os.path.join(self.test_data_dir, 'field_inference.all_absent.vcf')
        expected_result = os.path.join(self.test_data_dir, 'field_inference.expected.vcf')
        temp_descriptor, temp_output_path = tempfile.mkstemp(suffix='.vcf')
        with os.fdopen(temp_descriptor, 'w') as output_handle:
            # FIXME this is pretty hacky
